...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        num_classes = getattr(opt, 'num_classes', 4) if opt is not None else 4
...
class FeatureClassifier(nn.Module):
    """Classification head for feature maps.
    Takes a feature map [B, C, H, W] and outputs a continuous label value.
    """
    def __init__(self, feat_dim, spatial_size=None, hidden_dim=256):
        ...
        self.fc2 = nn.Linear(hidden_dim, 1)  # Output continuous value
    ...
    def forward(self, feat):
        ...
        Returns:
            Continuous label value [B, 1] in range [0, 1]
        """
        # Global Average Pooling
        x = self.global_pool(feat)  # [B, C, 1, 1]
...
class NLayerDiscriminator3(nn.Module):
    #https://github.com/commashy/SemCG-Stain/blob/main/models/networks_SemCG.py
    """Defines a PatchGAN discriminator with spectral normalization"""
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=4):
        """Construct a PatchGAN discriminator with spectral normalization
        
        Note: num_classes parameter is kept for compatibility but not used.
        The discriminator now outputs regression values (0, 0.33, 0.66, 1) instead of 4 classes.
        """
        ...
        if anti_alias:
            ...
            self.global_pool = nn.AdaptiveAvgPool2d(1)
            # Regression: output 1 value (0, 0.33, 0.66, 1) instead of 4 classes
            ...
            self.global_pool = nn.AdaptiveAvgPool2d(1)
            # Regression: output 1 value (0, 0.33, 0.66, 1) instead of 4 classes
            self.fc = spectral_norm(nn.Linear(dim, 1))
...
class MultiScaleDiscriminator(nn.Module):
    #https://github.com/commashy/SemCG-Stain/blob/main/models/networks_SemCG.py
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=4, num_scales=3, norm='none', activation='lrelu', pad_type='reflect'):
